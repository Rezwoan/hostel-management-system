<?php
/**
 * AJAX: Delete invoice
 * Usage: POST delete_invoice.php
 * Body: id=1
 */
session_start();
header("Content-Type: application/json");

if (!isset($_SESSION['user_id'])) {
    echo json_encode(["success" => false, "error" => "Unauthorized"]);
    exit;
}

require_once __DIR__ . '/../../Models/Database.php';

$id = isset($_POST['id']) ? (int)$_POST['id'] : 0;

if ($id <= 0) {
    echo json_encode(["success" => false, "error" => "Invalid invoice ID"]);
    exit;
}

$conn = dbConnect();

// Check for payments
$sql = "SELECT COUNT(*) as count FROM payments WHERE invoice_id = $id";
$result = mysqli_query($conn, $sql);
$count = mysqli_fetch_assoc($result)['count'];

if ($count > 0) {
    mysqli_close($conn);
    echo json_encode(["success" => false, "error" => "Cannot delete invoice with payments"]);
    exit;
}

$sql = "DELETE FROM student_invoices WHERE id = $id";
$result = mysqli_query($conn, $sql);
mysqli_close($conn);

if ($result) {
    echo json_encode(["success" => true, "message" => "Invoice deleted"]);
} else {
    echo json_encode(["success" => false, "error" => "Failed to delete invoice"]);
}
?>
